					 ...
					 const char *arg, int unset)
{
	...
	parse_result = parse_trailer(arg, cl_separators, &raw, &key, &val);

	if (parse_result == PARSE_FOUND_EMPTY_KEY) {
		error(_("ignoring --trailer argument '%s' with empty key"), arg);
		return 0;
	} else if (!key.len) {
		/*
		 * If we failed to parse the argument as a key (example:
		 * "--trailer foo" where there is no separator), then treat the
		 * entire arg ("foo") as a key (key alias), to be looked up
		 * against configuration (search for any trailer.foo.* configs).
		 */
		strbuf_addstr(&key, arg);
	}

	/* Lookup if the key matches something in the config */
	apply_matching_injector_from_config(&key, &conf, tsc);
			       ...
			       struct list_head *injectors)
{
	...
	trailer_block = parse_trailer_block(input.buf, opts, tsc);
	...
	if (!opts->only_input) {
		apply_trailer_injectors(injectors, trailer_block);
	}

	/* Print trailer block. */
	format_trailers(trailer_block, opts, tsc, &tb);
