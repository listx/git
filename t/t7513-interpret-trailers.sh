...
#
# (1) The initial "Fixes: Z" in the input is replaced with "Fixes: 53" because we
#     set "--if-exists replace".
#
# (2) Then when we read "--if-exists addIfDifferent" we will from no on reject
#     any newly supplied trailer if we already have the exact same trailer in the
#     (processed) input.
#
